...
//! @brief Construct a new Sequence object
//!
Sequence::Sequence(string sequenceString)
{
    string repeat = sequenceString.substr(0, sequenceString.find("{"));
    if (repeat == "inf")
    {
        this->repeat = -1;
    }
    ...
    string subsequenceString = sequenceString.substr(sequenceString.find("{") + 1, sequenceString.find_last_of("}") - sequenceString.find("{") - 1);
    int deep = 0;
    int index = 0;
    int start = index;
    ...
    while (index < subsequenceString.length())
    {
        ...
        else if (subsequenceString[index] == '}' || index == subsequenceString.length() - 1)
        {
            if (--deep <= 0)
            {
                string subsubsequenceString = subsequenceString.substr(start, index - start + 1);
                ...
                else
                {
                    ...
                    if (endStr != "")
                    {
                        subsequences.push_back(new Element(std::atof(startStr.c_str()), std::atol(timeStr.c_str()), std::atof(endStr.c_str())));
                    }
                    else if (timeStr != "")
                    {
                        subsequences.push_back(new Element(std::atof(startStr.c_str()), std::atol(timeStr.c_str())));
                    }
                    else if (startStr != "")
                    {
                        subsequences.push_back(new Element(std::atof(startStr.c_str())));
                    }
